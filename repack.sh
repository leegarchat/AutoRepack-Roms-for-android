...
unpack_payload(){
               python39 bin_payload/extract_payload-master/extract_payload.py '/'$pya_path'/payload.bin' extracted/
}
...
vendor_dfe(){
               ...
               vendor_size=$( expr $FILESIZE + 3000000)
               make_ext4fs -s -J -T -1 -S ./tmp/config/vendor/vendor_file_contexts -C ./tmp/config/vendor/vendor_fs_config -l $vendor_size -a /vendor "$OUT"vendor.img tmp/vendor
}
...
convert_img() {
               for file in $(ls -1 extracted/*.img | xargs -n1 basename)  do
                              system.img|product.img|system_ext.img|odm.img
                              if ! case "$file" in ($partition_covert) false; esac; then
                              echo -e "\e[1;32m-------------------------------------------------------------\e[0m"
...
for sparse in $(ls -1 $OUT/*.img | xargs -n1 basename) do
